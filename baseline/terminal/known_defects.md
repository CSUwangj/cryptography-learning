...
| --- | --- | --- | --- | --- |
| `duplicate-local-echo` | echo/output | `xterm.onData` locally writes every keystroke while `AttachAddon` also sends the same bytes to the Challenge, so input appears duplicated when the remote echoes. | Browser code never locally echoes input; Challenge output renders exactly once. | Fixed in #17: single AttachAddon path; no local `write` of input. |
